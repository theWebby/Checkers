    ...
    setupPieces() {
        ...
        for(var row = 0; row < 3; row++){
            for (var i = 0; i < (GRID_SIDE_LENGTH / 2); i++){
                if (this.isnNpc){
                    this.pieces.push(new Piece(i*2 + ((row + (FIRST_TILE == "black" ? 0 : 1)) % 2), row, this.color, this.isNpc))
                }
                else{
                    this.pieces.push(new Piece(i*2 + ((row + (FIRST_TILE == "black" ? 1 : 0)) % 2), GRID_SIDE_LENGTH - row - 1, this.color, this.isNpc))
                }
            }
        }
